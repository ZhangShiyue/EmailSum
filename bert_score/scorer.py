...
class BERTScorer:
    ...
    def __init__(
        ...
        if self.model_type.startswith("scibert"):
            self._tokenizer = AutoTokenizer.from_pretrained(cache_scibert(self.model_type), cache_dir=CACHE_DIR)
        else:
            self._tokenizer = AutoTokenizer.from_pretrained(self.model_type, cache_dir=CACHE_DIR)
